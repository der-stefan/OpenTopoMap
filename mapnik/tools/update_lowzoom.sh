...
psql -d gis -c "CREATE VIEW lowzoom_straitllabel AS SELECT way,name,'straitaxis'::text  AS label,st_length(way)*st_length(way)/10 as way_area FROM planet_osm_line WHERE  \"natural\" = 'strait' AND name IS NOT NULL;"
